...
class HisenseApiClient:
        ...
        try:
            ...
            for device_id, (device, parser) in devices_with_parsers.items():
                ...
                if "f_power_consumption" in parser.attributes:
                    try:
                        await self.client.update_power_consumption(device, self.token)
                ...
                try:
                    await self.client.update_self_check_data(device, self.token)
    ...
    async def async_control_device(
        ...
        try:
            response = await self.client.control_device(puid, properties, self.token)
